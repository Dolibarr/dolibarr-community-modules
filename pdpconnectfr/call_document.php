...
/**
 *  \file       call_document.php
 *  \ingroup    pdpconnectfr
 *  \brief      Tab for documents linked to Call
 */
...
$langs->loadLangs(array("pdpconnectfr@pdpconnectfr", "companies", "other", "mails"));
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->pdpconnectfr->multidir_output[$object->entity ? $object->entity : $conf->entity]."/call/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('pdpconnectfr', 'call', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
$modulepart = 'pdpconnectfr';
$param = '&id='.$object->id;
//$relativepathwithnofile='call/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'call/'.dol_sanitizeFileName($object->ref).'/';
