...
function GetInfoFromTrueType($file, $embed, $subset, $map)
{
	...
	if ($embed) {
		...
		if ($subset) {
			...
			$ttf->Subset($chars);
			$info['Data'] = $ttf->Build();
		} else $info['Data'] = file_get_contents($file);
	...
	foreach ($map as $c=>$v) {
		if ($v['name']!='.notdef') {
			if (isset($ttf->chars[$v['uv']])) {
				...
				$w = $ttf->glyphs[$id]['w'];
				$widths[$c] = round($k*$w);
			} else Warning('Character '.$v['name'].' is missing');
		}
	}
	$info['Widths'] = $widths;
	return $info;
...
function GetFontDescriptor($info)
{
	...
	else $desc['CapHeight'] = $info['Ascender'];
	// Flags
	$flags = 0;
...
function MakeFont($fontfile, $enc = 'cp1252', $embed = true, $subset = true)
{
	...
	elseif ($ext=='php') {
		...
		return;
	} else Error('Unrecognized font file extension: '.$ext);

	$map = LoadMap($enc);
	if ($type=='TrueType')
		$info = GetInfoFromTrueType($fontfile, $embed, $subset, $map);
	else $info = GetInfoFromType1($fontfile, $embed, $map);
