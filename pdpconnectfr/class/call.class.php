...
class Call extends CommonObject
{
	...
	public $fields = array(
		...
		"call_type" => array("type" => "varchar(50)", "label" => "call_type", "enabled" => "1", 'position' => 0, 'notnull' => 1, "visible" => "1", "comment" => "Call type (SyncDocs, SyncSoc, FetchDoc, FetchSoc...)"),
		"method" => array("type" => "varchar(10)", "label" => "method", "enabled" => "1", 'position' => 0, 'notnull' => 0, "visible" => "1", "comment" => "-- HTTP method (GET, POST, PUT, DELETE)"),
	...
	//  */
	// public $table_element_line = 'pdpconnectfr_callline';
	...
	//  */
	// protected $childtablesoncascade = array('pdpconnectfr_calldet');
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->pdpconnectfr->dir_output.'/call/'.$oldref;
				$dirdest = $conf->pdpconnectfr->dir_output.'/call/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->pdpconnectfr->dir_output.'/call/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/pdpconnectfr/call_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('PDPCONNECTFR_MYOBJECT_ADDON')) {
			$conf->global->PDPCONNECTFR_MYOBJECT_ADDON = 'mod_call_standard';
		}
