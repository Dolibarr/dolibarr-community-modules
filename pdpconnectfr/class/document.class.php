...
class Document extends CommonObject
{
	 ...
	 */
	public $table_element = 'pdpconnectfr_document';
	...
	public $fields = array(
		...
		"call_id" => array("type" => "varchar(50)", "label" => "call_id", "enabled" => "1", 'position' => 20, 'notnull' => 0, "visible" => "1", "comment" => "Reference to the original call"),
		...
		"flow_syntax" => array("type" => "varchar(50)", "label" => "flow_syntax", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "-2", "comment" => "Document syntax (FACTUR-X, etc.)"),
		"flow_profile" => array("type" => "varchar(50)", "label" => "flow_profile", "enabled" => "1", 'position' => 70, 'notnull' => 0, "visible" => "-2", "comment" => "Profile used (Basic, Cius, etc.)"),
		...
		"fk_element_id" => array("type" => "integer", "label" => "fk_element_id", "enabled" => "1", 'position' => 120, 'notnull' => 0, "visible" => "1",),
		"fk_element_type" => array("type" => "varchar(100)", "label" => "fk_element_type", "enabled" => "1", 'position' => 130, 'notnull' => 0, "visible" => "1",),
		...
		"cdar_lifecycle_code" => array("type" => "varchar(50)", "label" => "cdar_lifecycle_code", "enabled" => "1", 'position' => 101, 'notnull' => 0, "visible" => "-2",),
		"cdar_lifecycle_label" => array("type" => "varchar(255)", "label" => "cdar_lifecycle_label", "enabled" => "1", 'position' => 102, 'notnull' => 0, "visible" => "-2",),
		"cdar_reason_code" => array("type" => "varchar(50)", "label" => "cdar_reason_code", "enabled" => "1", 'position' => 103, 'notnull' => 0, "visible" => "-2",),
		"cdar_reason_desc" => array("type" => "varchar(255)", "label" => "cdar_reason_desc", "enabled" => "1", 'position' => 104, 'notnull' => 0, "visible" => "-2",),
		"cdar_reason_detail" => array("type" => "text", "label" => "cdar_reason_detail", "enabled" => "1", 'position' => 105, 'notnull' => 0, "visible" => "-2",),
		"ack_status" => array("type" => "varchar(50)", "label" => "ack_status", "enabled" => "1", 'position' => 80, 'notnull' => 0, "visible" => "-2", "comment" => "Acknowledgment status (Success, Error, Pending)"),
		"ack_reason_code" => array("type" => "varchar(255)", "label" => "ack_reason_code", "enabled" => "1", 'position' => 90, 'notnull' => 0, "visible" => "-2", "comment" => "Reason code if error"),
		"ack_info" => array("type" => "text", "label" => "ack_info", "enabled" => "1", 'position' => 100, 'notnull' => 0, "visible" => "-2", "comment" => "Additional information if error/warning"),
	);
	public $rowid;
	...
	//  */
	// public $table_element_line = 'pdpconnectfr_documentline';
	...
	//  */
	// public $fk_element = 'fk_document';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Documentline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Document', 'fk_element'=>'fk_document'));
	...
	//  */
	// protected $childtablesoncascade = array('pdpconnectfr_documentdet');

	// /**
	//  * @var DocumentLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->pdpconnectfr->dir_output.'/document/'.$oldref;
				$dirdest = $conf->pdpconnectfr->dir_output.'/document/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->pdpconnectfr->dir_output.'/document/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDocument")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Document").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/pdpconnectfr/document_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDocument");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DocumentLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_document:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('PDPCONNECTFR_MYOBJECT_ADDON')) {
			$conf->global->PDPCONNECTFR_MYOBJECT_ADDON = 'mod_document_standard';
		}

		if (getDolGlobalString('PDPCONNECTFR_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('PDPCONNECTFR_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('PDPCONNECTFR_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/pdpconnectfr/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_document');
			}
		}

		$modelpath = "core/modules/pdpconnectfr/doc/";
...
/**
 * Class DocumentLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DocumentLine extends CommonObjectLine
{
	// To complete with content of an object DocumentLine
	// We should have a field rowid, fk_document and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'document'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_document'
