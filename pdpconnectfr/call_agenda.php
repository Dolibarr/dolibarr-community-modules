...
/**
 *  \file       call_agenda.php
 *  \ingroup    pdpconnectfr
 *  \brief      Tab of events on Call
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->pdpconnectfr->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		//$cachekey = 'count_events_call_'.$object->id;
