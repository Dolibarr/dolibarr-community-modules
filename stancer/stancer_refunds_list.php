...
/**
 *   	\file       stancer_refunds_list.php
 *		\ingroup    stancer
 *		\brief      List page for stancer_refunds
 */
...
$action     = (string) GETPOST('action', 'aZ09') ? GETPOST('action', 'aZ09') : 'view'; // The action 'create'/'add', 'edit'/'update', 'view', ...
...
$toselect   = (string) GETPOST('toselect', 'array'); // Array of ids of elements selected into a list
...
$mode       = (string) GETPOST('mode', 'aZ'); // The output mode ('list', 'kanban', 'hierarchy', 'calendar', ...)
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Stancer_refunds';
	$objectlabel = 'Stancer_refunds';
...
$title = $langs->trans("StancerRefunds");
//$help_url = "EN:Module_Stancer_refunds|FR:Module_Stancer_refunds_FR|ES:Módulo_Stancer_refunds";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/stancer/stancer_refunds_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$topicmail = "SendStancer_refundsRef";
$modelmail = "stancer_refunds";
$objecttmp = new Stancer_refunds($db);
$trackid = 'xxxx'.$object->id;
include DOL_DOCUMENT_ROOT.'/core/tpl/massactions_pre.tpl.php';

if ($search_all) {
	$setupstring = '';
	// $fieldstosearchall stays empty as long as the block that fills it (just after its
	// declaration) is commented out. The loop is kept so the "search in all" display
	// works as soon as that block is enabled.
