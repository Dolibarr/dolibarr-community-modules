...
// M1: verify the online-payment security token BEFORE loading any object by ref.
// Without it, invoices/orders can be enumerated by (predictable) ref and arbitrary
// Stancer customers/payments created. Same scheme as the core newpayment.php and
// getOnlinePaymentUrl(): hash of PAYMENT_SECURITY_TOKEN + source + ref.
...
if ($source == 'invoice') {
	...
	//peut-être payée en plusieurs fois ? pour savoir ça il suffit de demander combien il reste à payer
	// note: probably duplicates the stancerMakeTAG() function ...
	...
	//payment 30% on order for example
	if (getDolGlobalString('STANCER_CB_ORDER_PARTIAL_PAY') != ''
	&& isset($object->deposit_percent) && $object->deposit_percent > 0 && $object->deposit_percent < 100) {
		...
		if ($partialAmount > 0 && $partialAmount < $amount) {
			dol_syslog("stancer newpayment.php : catch parent data and apply partial Amount to use : $partialAmount");
			$amount = $partialAmount;
		} else {
			dol_syslog("stancer newpayment.php : partial Amount not used : $partialAmount is <= 0 or not <= amount ($amount)");
		}
	}
} elseif ($source == 'propal') {
	$found = true;
...
if (preg_match('/^dopayment/', $action)) {			// If we choose/click on the payment mode
