...
/**
 *   	\file       stancer_payouts_list.php
 *		\ingroup    stancer
 *		\brief      List page for stancer_payouts
 */
...
$langs->loadLangs(array("stancer@stancer", "other"));
...
$hookmanager->initHooks(array('stancer_payoutslist')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Stancer_payouts';
	$objectlabel = 'Stancer_payouts';
...
if (getDolGlobalString('STANCER_IS_PROD', '0') == '0') {
	$message = "<h1>" . $langs->trans("NotAvailableInTestMode") . "</h1>";
	$action = "";
}
...
if ($action == "refreshone") {
	...
	if (!preg_match('/^[A-Za-z0-9_\-]{1,64}$/', $payoutIdToRefresh)) {
		...
		setEventMessages($langs->trans('StancerRefreshInvalidPayoutId'), null, 'errors');
	} else {
		dol_syslog("stancer refreshone payout_id=" . $payoutIdToRefresh);
		...
		stancerCleanupOrphanBankUrls();
		$refreshOutput = stancerRefreshOnePayout($payoutIdToRefresh, false);
		if (!empty($refreshOutput->error)) {
			setEventMessages($refreshOutput->error, null, 'errors');
		} else {
			setEventMessages($langs->trans('StancerPayoutRefreshed', $payoutIdToRefresh), null, 'mesgs');
		}
	}
	$action = 'list';
}
...
if ($action == "refreshall") {
	...
	// Clean any orphan bank_url left over from previous mis-reconciliations
	stancerCleanupOrphanBankUrls();
	stancerRefreshAllPayouts();

		// $res = $sp->fetch(null,null,$payout->getId());

		// $data = [
		// 	'stancer_id' => $payout->getId(),
		// 	'amount' => $payout->getAmount(),
		// 	'currency' => $payout->getCurrency(),
		// 	'date_paym' => $payout->getDateBank(),
		// 	'date_bank' => $payout->getDateBank(),
		// 	'fees' => $payout->getUniqueId(),
		// 	'payments' => $payout->getMethod(),
		// 	'refunds' => $payout->getCard(),
		// 	'disputes' => $payout->getSepa(),
		// 	'statement_description' => $payout->getCustomer(),
		// 	'status' => $payout->getRefunds(),
		// 	'created' => $payout->getStatus(),
		// ];
		// $sp->fillData($data);
		...
		//print "<p>liste : " . $p->getId() . ", status=" . $p->getStatus() ."</p>";
		// $data = [
		// 	'stancer_id' => $payout->getId(),
		// 	'amount' => $payout->getAmount(),
		// 	'currency' => $payout->getCurrency(),
		// 	'description' => $payout->getDescription(),
		// 	'order_id' => $payout->getOrderId(),
		// 	'unique_id' => $payout->getUniqueId(),
		// 	'method' => $payout->getMethod(),
		// 	'card' => $payout->getCard(),
		// 	'sepa' => $payout->getSepa(),
		// 	'customer' => $payout->getCustomer(),
		// 	'refunds' => json_encode($payout->getRefunds()),
		// 	'status' => $payout->getStatus(),
		// 	'response' => $payout->getResponse(),
		// 	'capture' => $payout->getCapture(),
		// 	// 'created' => $payout->getCreated(),
		// 	// 'date_bank' => $payout->getDateBank(),
		// 	'return_url' => $payout->getReturnUrl(),
		// 	'live_mode' => getDolGlobalString('STANCER_IS_PROD'),
		// ];
		// $p->fillData($data);
		// $p->update($user);
...
//$help_url = "EN:Module_Stancer_payouts|FR:Module_Stancer_payouts_FR|ES:Módulo_Stancer_payouts";
$help_url = '';
$title = $langs->trans("Stancer_payouts");
$morejs = array();
$morecss = array();
...
foreach ($search as $key => $val) {
	if (array_key_exists($key, $object->fields)) {
		...
		if ($search[$key] != '') {
			...
			if (in_array($key, array('amount', 'fees', 'amount_net'))) {
...
$topicmail = "SendStancer_payoutsRef";
$modelmail = "stancer_payouts";
$objecttmp = new Stancer_payouts($db);
...
// Totals: amount/fees/amount_net are stored in cents, convert to euros.
