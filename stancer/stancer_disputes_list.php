...
/**
 *   	\file       stancer_disputes_list.php
 *		\ingroup    stancer
 *		\brief      List page for stancer_disputes
 */
...
$action     = (string) GETPOST('action', 'aZ09') ? GETPOST('action', 'aZ09') : 'view';
$massaction = (string) GETPOST('massaction', 'alpha');
...
$contextpage = (string) GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : str_replace('_', '', dol_basename(dirname(__FILE__)).preg_replace('/\.php$/', '', pathinfo(__FILE__, PATHINFO_BASENAME)));
...
$object = new Stancer_disputes($db);
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Stancer_disputes';
	$objectlabel = 'Stancer_disputes';
...
$title = $langs->trans("StancerDisputes");
$help_url = '';
$morejs = array();
$morecss = array();
...
foreach ($search as $key => $val) {
	if (array_key_exists($key, $object->fields)) {
		...
		if ((strpos($object->fields[$key]['type'], 'integer:') === 0) || (strpos($object->fields[$key]['type'], 'sellist:') === 0) || !empty($object->fields[$key]['arrayofkeyval'])) {
			...
			// Use exact string match for varchar fields with arrayofkeyval
		...
		if ($search[$key] != '') {
			// Amount and fee are stored in cents, convert EUR input to cents for search
...
$topicmail = "SendStancer_disputesRef";
$modelmail = "stancer_disputes";
$objecttmp = new Stancer_disputes($db);
...
$orderIdCache = array(); // Cache for getObjectFromOrderID to avoid repeated DB lookups
while ($i < $imaxinloop) {
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				if ($key == 'status') {
					...
					print dol_escape_htmltag($object->dispute_id);
				} elseif ($key == 'payment_id' && !empty($object->payment_id)) {
					// Link to payment list filtered by this payment ID
					$paymentUrl = dol_buildpath('/stancer/stancer_payments_list.php', 1) . '?search_stancer_id=' . urlencode($object->payment_id);
					print '<a href="' . $paymentUrl . '">' . dol_escape_htmltag($object->payment_id) . '</a>';
				} elseif ($key == 'order_id' && !empty($object->order_id)) {
					// Resolve order_id to invoice/propal/order and build link (cached)
					$oid = $object->order_id;
					if (!array_key_exists($oid, $orderIdCache)) {
						$orderIdCache[$oid] = getObjectFromOrderID($oid);
					}
					$linkedObj = $orderIdCache[$oid];
					if ($linkedObj !== null && !empty($linkedObj->id)) {
						print $linkedObj->getNomUrl(1);
					} else {
