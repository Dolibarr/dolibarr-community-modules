	...
	public function saveConfFromPost($noMessageInUpdate = false)
	{
		...
		if (!$error) {
			...
			if (empty($noMessageInUpdate)) {
				setEventMessages($this->langs->trans("SetupSaved"), []);
			}
			return 1;
		} else {
			$this->db->rollback();
			if (empty($noMessageInUpdate)) {
				setEventMessages($this->langs->trans("SetupNotSaved"), [], 'errors');
			}
			return -1;
		}
	}
...
class FormSetupItem
{
	...
	public function generateInputFieldHtml()
	{
		...
		// @phpstan-ignore-next-line Parameter #3 $width of class DolEditor constructor expects int, string given.
