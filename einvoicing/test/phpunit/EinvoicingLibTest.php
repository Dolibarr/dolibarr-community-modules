...
class EinvoicingLibTest extends CommonClassTest
{
	...
	/**
	 * A flow id says nothing on its own: the entry of the selection list carries what tells two flows
	 * apart - the invoice number, the date, the vendor and the reason the flow is waiting.
	 *
	 * @return void
	 */
	public function testAFlowIsNamedByMoreThanItsIdentifier()
	{
		$label = einvoicingFlowChoiceLabel(array(
			'flowid' => 'i_20260918_00000123',
			'ref' => 'FA2409-0042',
			'date' => dol_mktime(0, 0, 0, 9, 18, 2026),
			'socname' => 'ACME SARL',
			'reason' => 'PRODUCT_NOT_FOUND',
			'pending' => 1,
		));

		$this->assertStringContainsString('i_20260918_00000123', $label, 'The entry does not carry the flow id');
		$this->assertStringContainsString('FA2409-0042', $label, 'The entry does not carry the invoice number');
		$this->assertStringContainsString('ACME SARL', $label, 'The entry does not carry the vendor');
		$this->assertStringContainsString(einvoicingSyncPendingReasonLabel('PRODUCT_NOT_FOUND'), $label, 'The entry does not say why the flow is waiting');
	}

	/**
	 * What a flow does not carry is left out, rather than shown as an empty part: a document received
	 * without an invoice number, or a queued flow whose issuer is unknown, is the normal case.
	 *
	 * @return void
	 */
	public function testAFlowWithoutAnyDetailIsNamedByItsIdentifierAlone()
	{
		$label = einvoicingFlowChoiceLabel(array(
			'flowid' => 'i_20260918_00000124',
			'ref' => '',
			'date' => 0,
			'socname' => '',
			'reason' => '',
		...
		$this->assertEquals('i_20260918_00000124', $label, 'An entry with nothing to add must be the flow id alone');
	}

	/**
	 * A reason code is shown translated, and an unknown one is shown as it stands rather than dropped:
	 * a flow waiting for a reason this list does not know about still has to say something.
	 *
	 * @return void
	 */
	public function testAnUnknownReasonCodeIsShownAsItStands()
	{
		$this->assertEquals('', einvoicingSyncPendingReasonLabel(''), 'An absent reason code must produce nothing');
		$this->assertEquals('SOMETHING_ELSE', einvoicingSyncPendingReasonLabel('SOMETHING_ELSE'), 'An unknown reason code must be shown as it stands');
	}
}
