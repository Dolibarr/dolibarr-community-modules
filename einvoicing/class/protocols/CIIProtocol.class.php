...
class CIIProtocol extends AbstractProtocol
{
	...
	/**
	 * @const string Profile used to generate XML. Every place buildXML() branches on the profile now
	 * handles EXTENDEDFR, so switching this constant is enough to emit EXTENDED-CTC-FR. Note that on
	 * the Factur-X protocol the PDF attachment step additionally needs horstoeko/zugferd to know that
	 * guideline, which it does not yet.
	 */
	 ...
	 * @param ''|'MINIMUM'|'BASICWL'|'BASIC'|'EN16931'|'EXTENDED'|'EXTENDEDFR' 	$profile	Profile ('MINIMUM', 'BASICWL', 'BASIC', 'EN16931', 'EXTENDED', 'EXTENDEDFR')
	 *
	...
	private function buildLineItem(DOMDocument $doc, array $line, string $profile)
	{
		...
		if (!empty($line['isDepositLine'])) {
			...
			// EXTENDED only: EXTENDED-CTC-FR forbids the issue date on a line-level referenced document (CII-FREXT-DT-027).
			// The date of the deposit invoice is still carried at header level by ram:InvoiceReferencedDocument (BT-26).
