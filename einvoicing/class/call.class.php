...
/**
 * \file        class/call.class.php
 * \ingroup     einvoicing
 * \brief       This file is a CRUD class file for Call (Create/Read/Update/Delete)
 */
 ...
 * Class for Call
 */
class Call extends CommonObject
{
	/**
	 ...
	 */
	public $TRIGGER_PREFIX = 'CALL';	// Will be used to build trgiger keys 'EINVOICING_MYOBJECT_MODIFY', ...

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'einvoicing_call';

	/**
	 * @var string 		If permission must be checked with hasRight('einvoicing', 'read') and not hasright('einvoicing', 'document', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'einvoicing';

	/**
	 * @var string 		String with name of icon for call. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'call@einvoicing' if picto is file 'img/object_call.png'.
	 */
	...
	//  */
	// public $table_element_line = 'einvoicing_callline';
	...
	//  */
	// public $fk_element = 'fk_call';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Callline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Call', 'fk_element'=>'fk_call'));
	...
	//  */
	// protected $childtablesoncascade = array('einvoicing_calldet');

	// /**
	//  * @var CallLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function createFromClone(User $user, $fromid)
	{
		...
		if (property_exists($object, 'label')) {   // @phan-suppress-next-line PhanUndeclaredProperty
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->einvoicing->dir_output.'/call/'.$oldref;
				$dirdest = $conf->einvoicing->dir_output.'/call/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->einvoicing->dir_output.'/call/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_FAILED, $notrigger, 'EINVOICING_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'EINVOICING_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_SUCCESS, $notrigger, 'EINVOICING_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Call").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/einvoicing/call_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
	...
	public function getKanbanView($option = '', $arraydata = null)
	{
		...
		if (property_exists($this, 'thirdparty') && is_object($this->thirdparty)) {
			$return .= '<br><div class="info-box-ref tdoverflowmax150">'.$this->thirdparty->getNomUrl(1).'</div>';  // @phan-suppress-current-line PhanUndeclaredProperty
		}
	...
	public function getLinesArray()
	{
		...
		$objectline = new CallLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_call:=:'.((int) $this->id).')');
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_call');
			}
		}

		$modelpath = "core/modules/einvoicing/doc/";
...
/**
 * Class CallLine. You can also remove this and generate a CRUD class for lines objects.
 */
class CallLine extends CommonObjectLine
{
	// To complete with content of an object CallLine
	// We should have a field rowid, fk_call and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'call'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_call'
