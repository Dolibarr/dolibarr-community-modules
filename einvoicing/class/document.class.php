...
class Document extends CommonObject
{
	 ...
	 */
	public $TRIGGER_PREFIX = 'EINVOICING_MYOBJECT';	// Will be used to build trgiger keys 'EINVOICING_MYOBJECT_MODIFY', ...

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'einvoicing_document';
	...
	/**
	 * @var string 		String with name of icon for document. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'document@einvoicing' if picto is file 'img/object_document.png'.
	 */
	...
	//  */
	// public $table_element_line = 'einvoicing_documentline';
	...
	//  */
	// protected $childtablesoncascade = array('einvoicing_documentdet');
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->einvoicing->dir_output.'/document/'.$oldref;
				$dirdest = $conf->einvoicing->dir_output.'/document/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->einvoicing->dir_output.'/document/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'EINVOICING_MYOBJECT_UNVALIDATE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'EINVOICING_MYOBJECT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/einvoicing/document_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
