...
class InterfaceEInvoicingTriggers extends DolibarrTriggers
{
				...
				if ($paidAmount > 0 && SupplierInvoiceHelper::isEInvoice($object->id)) {
					...
					if (!$einvoicing->hasSentStatusMessage($object->id, $object->element, EInvoicing::STATUS_PAYMENT_SENT)) {
						...
						if ($result['res'] > 0) {
							...
							// undo a real payment (see sendCashedInStatus() for the same reasoning).
	 ...
	 *
	 * The reform only requires the payment data for the operations whose VAT is due on collection: services
	 * and down payments. It is out of scope for a seller who opted for the "TVA d'apres les debits" scheme
	 * (VAT then falls due on invoicing) - EINVOICING_VAT_ON_DEBITS. For a goods invoice the status is merely
	 * optional, so it is still sent by default (over-reporting is allowed, under-reporting is not);
	 * EINVOICING_PAID_STATUS_SERVICES_ONLY restricts it to the invoices that really owe it.
	 *
	 * @param  Facture $invoice Invoice that has been cashed in
	...
	{
		if (getDolGlobalInt('EINVOICING_VAT_ON_DEBITS')) {
			return false;
		}

		if (!getDolGlobalInt('EINVOICING_PAID_STATUS_SERVICES_ONLY')) {
			return true;
		}

		if ($invoice->type == Facture::TYPE_DEPOSIT) {	// VAT on a down payment is always due on collection
			return true;
		}
		...
		foreach ($invoice->lines as $line) {
			if ($line->product_type == 1) {		// Product::TYPE_SERVICE, without requiring the product class here
				return true;
			}
		}
