...
/**
 * 	\defgroup   einvoicing     Module EInvoicing
 *  \brief      EInvoicing module descriptor.
 *
 *  \file       htdocs/einvoicing/core/modules/modEInvoicing.class.php
 *  \ingroup    einvoicing
...
/**
 *  Description and activation class for module EInvoicing
 */
class modEInvoicing extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleEInvoicingName' not found (EInvoicing is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));

		// DESCRIPTION_FLAG
		// Module description, used if translation string 'ModuleEInvoicingDesc' not found (EInvoicing is name of module).
		$this->description = "EInvoicingDescription";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "EInvoicingDescription";
		...
		$this->picto = 'einvoicing.png@einvoicing';
		...
		// Example: this->dirs = array("/einvoicing/temp","/einvoicing/subdir");
		$this->dirs = array("/einvoicing/temp");

		// Config pages. Put here list of php page, stored into einvoicing/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@einvoicing");
		...
		//$this->automatic_activation = array('FR'=>'EInvoicingWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		...
		// $this->tabs[] = array('data' => 'objecttype:+tabname1:Title1:mylangfile@einvoicing:$user->hasRight('einvoicing', 'document', 'read'):/einvoicing/mynewtab1.php?id=__ID__');
		// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		// $this->tabs[] = array('data' => 'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@einvoicing:$user->hasRight('othermodule', 'otherobject', 'read'):/einvoicing/mynewtab2.php?id=__ID__',
		...
		/* Example:
		 ...
		 'tabcond' => array(isModEnabled('einvoicing'), isModEnabled('einvoicing'), isModEnabled('einvoicing')),
		...
		$this->cronjobs = array(
			  0 => array(
				  'label' => 'EInvoicingDocumentSync',
				  'jobtype' => 'method',
				  'class' => '/einvoicing/class/document.class.php',
				  'objectname' => 'Document',
				  'method' => 'cronSyncFlows',
				  'parameters' => '',
				  'comment' => 'EInvoicingDocumentSyncDescription',
				  'frequency' => 1,
				  'unitfrequency' => 3600,
				  'status' => 0,
		...
		/*$this->menu[$r++] = array(
			...
			'titre' => 'ModuleEInvoicingName',
			'prefix' => img_picto('', $this->picto, 'class="pictofixedwidth valignmiddle"'),
			'mainmenu' => 'einvoicing',
			'leftmenu' => '',
			'url' => '/einvoicing/einvoicingindex.php',
			'langs' => 'einvoicing@einvoicing', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position' => 1000 + $r,
			'enabled' => 'isModEnabled("einvoicing")', // Define condition to show or hide menu entry. Use 'isModEnabled("einvoicing")' if entry must be visible if module is enabled.
			'perms' => '1', // Use 'perms'=>'$user->hasRight("einvoicing", "call", "read")' if you want your menu with a permission rules
			'target' => '',
		...
		$this->menu[$r++] = array(
			...
			'prefix' => img_picto('', 'einvoicing.png@einvoicing', 'class="pictofixedwidth valignmiddle paddingright"'),
			'mainmenu' => 'billing',
			'leftmenu' => 'einvoicing_billing',
			'url' => '/einvoicing/einvoicingindex.php',
			'langs' => 'einvoicing@einvoicing',
			'position' => 1000,
		...
		$this->menu[$r++] = array(
			...
			'langs' => 'einvoicing@einvoicing',
			'position' => 1004,
			'enabled' => 'isModEnabled("einvoicing") && !getDolGlobalString("EINVOICING_ONLY_GENERATE") && (getDolGlobalInt("EINVOICING_ENABLE_MANUAL_ACTION_QUEUE") || getDolGlobalInt("EINVOICING_ENABLE_POSTPONE_FLOWS"))',
			'perms' => '$user->hasRight("einvoicing", "read")',
			'target' => '',
			'user' => 2,
			'object' => '',
		...
		/*
		$this->menu[$r++]=array(
			'fk_menu' => 'fk_mainmenu=einvoicing,fk_leftmenu=call',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type' => 'left',			                // This is a Left menu entry
			'titre' => 'New_Call',
			'mainmenu' => 'einvoicing',
			'leftmenu' => 'einvoicing_call_new',
			'url' => '/einvoicing/call_card.php?action=create',
			'langs' => 'einvoicing@einvoicing',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position' => 1000 + $r,
			'enabled' => 'isModEnabled("einvoicing")', // Define condition to show or hide menu entry. Use 'isModEnabled("einvoicing")' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms' => '$user->hasRight("einvoicing", "call", "write")'
			'target' => '',
			'user' => 2,				                // 0=Menu for internal users, 1=external users, 2=both
			'object' => 'Call'
		);
		$this->menu[$r++]=array(
			...
			'titre' => 'List_Call',
			'mainmenu' => 'einvoicing',
			'leftmenu' => 'einvoicing_call_list',
			'url' => '/einvoicing/call_list.php',
			'langs' => 'einvoicing@einvoicing',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position' => 1000 + $r,
			'enabled' => 'isModEnabled("einvoicing")', // Define condition to show or hide menu entry. Use 'isModEnabled("einvoicing")' if entry must be visible if module is enabled.
			'perms' => '$user->hasRight("einvoicing", "call", "read")'
			'target' => '',
			'user' => 2,				                // 0=Menu for internal users, 1=external users, 2=both
			'object' => 'Call'
		);
		*/
		...
		$keyforclass = 'Call'; $keyforclassfile='/einvoicing/class/call.class.php'; $keyforelement='call@einvoicing';
		include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		//$this->export_fields_array[$r]['t.fieldtoadd']='FieldToAdd'; $this->export_TypeFields_array[$r]['t.fieldtoadd']='Text';
		//unset($this->export_fields_array[$r]['t.fieldtoremove']);
		//$keyforclass = 'CallLine'; $keyforclassfile='/einvoicing/class/call.class.php'; $keyforelement='callline@einvoicing'; $keyforalias='tl';
		//include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		$keyforselect='call'; $keyforaliasextra='extra'; $keyforelement='call@einvoicing';
		include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$keyforselect='callline'; $keyforaliasextra='extraline'; $keyforelement='callline@einvoicing';
		//include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		...
		$this->export_sql_end[$r]  =' FROM '.$this->db->prefix().'einvoicing_call as t';
		//$this->export_sql_end[$r]  .=' LEFT JOIN '.$this->db->prefix().'einvoicing_call_line as tl ON tl.fk_call = t.rowid';
		...
		$this->import_tables_array[$r] = array('t' => $this->db->prefix().'einvoicing_call', 'extra' => $this->db->prefix().'einvoicing_call_extrafields');
		...
		$this->import_fieldshidden_array[$r] = array('extra.fk_object' => 'lastrowid-'.$this->db->prefix().'einvoicing_call');
		...
		$this->import_convertvalue_array[$r] = array(
			't.ref' => array(
				'rule'=>'getrefifauto',
				'class'=>(!getDolGlobalString('EINVOICING_MYOBJECT_ADDON') ? 'mod_call_standard' : getDolGlobalString('EINVOICING_MYOBJECT_ADDON')),
				'path'=>"/core/modules/einvoicing/".(!getDolGlobalString('EINVOICING_MYOBJECT_ADDON') ? 'mod_call_standard' : getDolGlobalString('EINVOICING_MYOBJECT_ADDON')).'.php',
				'classobject'=>'Call',
				'pathobject'=>'/einvoicing/class/call.class.php',
			),
	...
	public function init($options = '')
	{
		...
		//$result = $this->_load_tables('/install/mysql/', 'einvoicing');
		$result = $this->_load_tables('/einvoicing/sql/');
		if ($result < 0) {
		...
		$result = $extrafields->update(
			...
			'', //$computerd
			'', //$entity
			'einvoicing@einvoicing', //$langfile
			'getDolGlobalInt("EINVOICING_USE_CHORUS")',
			0, //$totalizable
			0, //$printable
			array() //$moreparams
		);
