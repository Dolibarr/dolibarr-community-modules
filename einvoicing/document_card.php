...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/einvoicing/document_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/einvoicing/document_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	*/
	...
	$triggersendname = 'EINVOICING_MYOBJECT_SENTBYMAIL';
...
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-einvoicing page-card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/einvoicing/document_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend' && 1 == 0) {  // @phan-suppress-current-line PhanPluginBothLiteralsBinaryOp
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/einvoicing/document_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->einvoicing->dir_output;
	$trackid = 'document'.$object->id;
