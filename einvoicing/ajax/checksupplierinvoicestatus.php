...
/**
 *       \file       htdocs/einvoicing/ajax/checksupplierinvoicestatus.php
...
dol_syslog("Call ajax einvoicing/ajax/checksupplierinvoicestatus.php");
...
if ($objectID) {
	...
	if (empty($lcId)) {
		print json_encode(['statuslabel' => 'No flow yet']);
		exit;
	}
	...
	if ($response['status_code'] == 200 || $response['status_code'] == 202) {
		...
		$htmlstatusvalidationLabel = ' : ' . $einvoicing->getStatusLabel($einvoicing->getDolibarrStatusCodeFromPdpLabel($statusvalidationlabel)) . ' ' .$picto;
