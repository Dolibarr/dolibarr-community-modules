...
dol_include_once('/einvoicing/lib/einvoicing_call.lib.php');
...
$permissiontodelete = $user->hasRight('einvoicing', 'delete') || ($permissiontoadd);
...
$upload_dir = $conf->einvoicing->multidir_output[isset($object->entity) ? $object->entity : 1].'/call';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/einvoicing/call_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/einvoicing/call_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/einvoicing/call_list.php', 1).'?restore_lastsearch_values=1">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			// Clone (dolGetButtonAction() greys it without the right)
	...
	if ($action != 'presend') {
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/einvoicing/call_agenda.php', 1).'?id='.$object->id);
