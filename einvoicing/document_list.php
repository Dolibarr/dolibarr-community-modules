...
if (empty($reshook)) {
	...
	$uploaddir = $conf->einvoicing->dir_output;
...
// --------------------------------------------------------------------
$sql = "SELECT";
// Fields of the table for the SELECT. 'recap' and 'thirdparty' are virtual columns of this list - they
// have no column in the table - and 'xml_data' is never displayed, so the three must stay out of the query.
$columns = array('recap', 'xml_data', 'thirdparty');
if ((float) DOL_VERSION < 18) {
	...
	$selectFields = $object->getFieldList('t', $columns);
}
$sql .= " ".$selectFields;
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/einvoicing/document_card.php', 1).'?id='.((int) $id));
	exit;
}
...
if ($provider) {
	...
	if (getDolGlobalInt('EINVOICING_SYNC_MARGIN_TIME_HOURS') && !$syncfromdate && !GETPOSTDATE('last_sync_datetime', 'getpost', 'tzuserrel')) {
		$gmtdatetosuggest -= (getDolGlobalInt('EINVOICING_SYNC_MARGIN_TIME_HOURS') * 3600);
	}
	...
	if ($conf->browser->layout != 'phone' && getDolGlobalInt('EINVOICING_SYNC_MARGIN_TIME_HOURS')) {
		print $form->textwithpicto('', $langs->transnoentitiesnoconv("EINVOICING_SYNC_MARGIN_TIME_HOURS").': '.getDolGlobalInt('EINVOICING_SYNC_MARGIN_TIME_HOURS').'<br><br>'.$langs->transnoentitiesnoconv("EINVOICING_SYNC_MARGIN_TIME_HOURS_HELP"));
	}
	...
	if (getDolGlobalInt('EINVOICING_FLOWS_SYNC_CALL_LIMIT')) {
		...
		print '<td>';
		print '<input type="number" id="maxflows" name="maxflows" class="maxwidth75 right" min="1" step="1" value="'.(GETPOSTINT('maxflows') ? GETPOSTINT('maxflows') : getDolGlobalInt('EINVOICING_FLOWS_SYNC_CALL_SIZE', 100)).'" required> ';
...
// Sync results
if ($action == 'confirm_sync' && getDolGlobalString('EINVOICING_PDP') && $confirm == 'yes' && !empty($sync_result)) {
	if (isset($provider)) {
