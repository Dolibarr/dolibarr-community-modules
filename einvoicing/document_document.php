...
/**
 *  \file       document_document.php
 *  \ingroup    einvoicing
 *  \brief      Tab for documents linked to Document
 */
...
$langs->loadLangs(array("einvoicing@einvoicing", "companies", "other", "mails"));
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->einvoicing->multidir_output[$object->entity ? $object->entity : $conf->entity]."/document/".get_exdir(0, 0, 0, 1, $object);
}
...
// Header
// ------
$title = $langs->trans("Document")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-einvoicing page-card_document');
...
$modulepart = 'einvoicing';
$param = '&id='.$object->id;
//$relativepathwithnofile='document/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'document/'.dol_sanitizeFileName($object->ref).'/';
